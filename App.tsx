...
const App: React.FC = () => {
  ...
  const handleFileChange = useCallback(async (file: File | null) => {
    ...
    try {
      ...
      if (!text.trim()) {
        setError("Could not extract text from the PDF, or the PDF is empty. Please try a different PDF.");
        setPdfText('');
        setIsLoading(false);
        setCurrentStep('upload');
        return;
      ...
      setError(err instanceof Error ? `Failed to process PDF: ${err.message}. Please ensure it's a valid PDF.` : "An unknown error occurred while processing the PDF.");
      setPdfText('');
      setPdfFile(null);
      setPdfFileName('');
      setCurrentStep('upload');
  ...
  const handleGenerateQuiz = useCallback(async () => {
    ...
    try {
      const generatedQuiz = await generateQuizFromText(ai, pdfText, difficulty, GEMINI_MODEL_NAME);
      if (generatedQuiz && generatedQuiz.length > 0) {
        setQuiz(generatedQuiz);
        setCurrentStep('quiz_ready');
      } else {
        setError("The AI couldn't generate a quiz from the provided text. The PDF content might not be suitable for quiz generation, or the AI service might be temporarily unavailable.");
        setCurrentStep('upload');
      }
    } catch (err) {
      console.error("Error generating quiz:", err);
      setError(err instanceof Error ? `Failed to generate quiz: ${err.message}` : "An unknown error occurred while generating the quiz.");
      setCurrentStep('upload');
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="min-h-screen flex flex-col items-center p-4 sm:p-6 lg:p-8 pt-10 bg-slate-900">
      ...
      <main className="w-full max-w-2xl bg-slate-800 shadow-2xl rounded-xl p-6 sm:p-8">
        {isLoading && <Loader message={
          currentStep === 'processing_pdf' ? "Analyzing PDF..." : 
          currentStep === 'generating_quiz' ? "Crafting your quiz with AI..." : 
          "Loading..."
        } />}
        ...
        {!isLoading && !quiz && (
          <div className="space-y-6">
            <div>
              ...
              <FileUpload onFileChange={handleFileChange} disabled={isLoading} currentFileName={pdfFileName} />
              {pdfFileName && !pdfText && !isLoading && !error && (
                 <p className="mt-2 text-sm text-yellow-400">Processing "{pdfFileName}" failed or text extraction yielded no content.</p>
              )}
               {pdfFileName && pdfText && !isLoading && !error && (
                 <p className="mt-2 text-sm text-green-400">Successfully processed "{pdfFileName}". Ready for quiz generation.</p>
              )}
            </div>
            ...
            {pdfText && (
              <>
                <div>
                  <label htmlFor="difficulty-selector" className="block text-sm font-medium text-slate-300 mb-1">
                    2. Select Difficulty
                  </label>
                  <DifficultySelector
                ...
                >
                  {isLoading && currentStep === 'generating_quiz' ? 'Generating...' : 'Generate Quiz'}
                </button>
              </>
            )}
          </div>
        ...
        {quiz && !isLoading && (
          <div className="space-y-6">
            <h2 className="text-2xl font-semibold text-sky-400 text-center">Your Custom Quiz!</h2>
      ...
      <footer className="mt-12 text-center text-sm text-slate-500">
        <p>&copy; {new Date().getFullYear()} AI Quiz Master. Powered by Gemini.</p>
      </footer>
    </div>
  );
};
